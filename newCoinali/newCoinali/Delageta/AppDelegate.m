...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [network GetsThePublicKeyInterfaceSuccess:^(NSDictionary *obj) {
        NSString *publishKey = obj[@"result"][@"data"];
        
        NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
        
        if (publishKey.length>0) {
            [user setObject:publishKey forKey:@"publishKey"];
        }
       // [IHUtility addSucessView:MyLocalized(@"Public key success") type:1];
    } failure:^(NSDictionary *obj2) {
        [IHUtility addSucessView:MyLocalized(@"Public key failure") type:1];
